  ...
  try {
    ...
    if (sortedBy === 'title'){
      result = await databases.listDocuments(
        databaseId,
        notesCollectionId,
        [Query.equal('creator', userId), Query.orderAsc(sortedBy)]
    )
    } else {
      result = await databases.listDocuments(
        databaseId,
        notesCollectionId,
        [Query.equal('creator', userId), Query.orderDesc(sortedBy)]
    )
    }
...
export const createNote = async (form) => {
  try {
    await databases.createDocument(
      databaseId, notesCollectionId, ID.unique(), {
        title: form.title,
        text: form.text,
        color: form.color,
        creator: form.userId
...
export const updateNote = async (form, noteId) => {
  try {
    await databases.updateDocument(
      databaseId, notesCollectionId, noteId, {
        title: form.title,
        text: form.text,
        color: form.color,
      })
...
export const updateCrossedOut = async (listId, crossedOutList) => {
  try {
    await databases.updateDocument(
      databaseId, listsCollectionId, listId, {
...
export const createList = async (form) => {
  try {
    await databases.createDocument(
      databaseId, listsCollectionId, ID.unique(), {
        title: form.title,
        list: form.list,
        color: form.color,
        creator: form.userId
...
export const uploadFile = async (file) => {
  ...
  try {
    ...
    const fileUrl = storage.getFilePreview(storageId, uploadedFile.$id, 2000, 2000, 'top', 100)
    return fileUrl
  } catch (error) {
    throw new Error()
  }
...
export const setImage = async (form) => {
  try {
    ...
    const newImage = await databases.createDocument(
      databaseId, imagesCollectionId, ID.unique(), {
        title: form.title,
        image: url,
        creator: form.userId,
        color: form.color
